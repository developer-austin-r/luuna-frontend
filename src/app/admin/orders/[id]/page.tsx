...
export default function OrderDetailsPage() {
    ...
    return (
      <div className="space-y-6">
        ...
        <Card className="text-center py-12">
          <p className="text-sm font-semibold text-text-custom/60">Order file not found.</p>
  ...
  const orderItems = [
    { id: 'item-1', name: 'Classic Leather Tote Bag', sku: 'BG-LTH-01', price: 159.00, quantity: 2, total: 318.00 }
  ];

  // Pricing calculations
  const subtotal = orderItems.reduce((sum, item) => sum + item.total, 0);
  const tax = subtotal * 0.08;
  const shippingCost = order.shippingMethod === 'Express Shipping' ? 14.99 : 5.99;
  const total = subtotal + tax + shippingCost;

  // Workflow Progress Timeline
  const workflowSteps = [
    { label: 'Placed', active: true, done: true },
    { label: 'Paid', active: order.paymentStatus === 'paid', done: order.paymentStatus === 'paid' },
    { label: 'Shipped', active: order.status === 'shipped' || order.status === 'delivered', done: order.status === 'shipped' || order.status === 'delivered' },
  ...
  const handleRefundSubmit = (data: RefundFormValues) => {
    dispatch(updateOrderPaymentStatus({ id: order.id, paymentStatus: 'refunded' }));
    ...
    alert(`Refund of $${data.refundAmount} has been processed for ${order.customerName}.`);
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-2xl font-bold text-text-custom mt-1">Order details</h1>
          </div>
        </div>

        <div className="flex gap-2 self-start sm:self-auto shrink-0">
          <Button variant="outline" size="sm" onClick={() => setInvoiceModalOpen(true)} className="flex items-center gap-1.5">
            <FileText className="w-4 h-4 text-primary" />
            Invoice
          </Button>
          {order.paymentStatus !== 'refunded' && (
            <Button variant="outline" size="sm" onClick={() => setRefundModalOpen(true)} className="flex items-center gap-1.5 border-red-200 text-red-500 hover:bg-red-50">
              <RefreshCcw className="w-4 h-4" />
              Refund Order
            </Button>
          )}
          <span className="flex items-center"><StatusBadge status={order.status} /></span>
        </div>
      </div>
          ...
          {workflowSteps.map((step, idx) => (
            <React.Fragment key={idx}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className={`text-xs font-bold ${step.done ? 'text-primary' : 'text-text-custom/50'}`}>
                    Order {step.label}
                  </p>
          ...
          <Card title="Ordered Items">
            ...
            <div className="border-t border-border-custom pt-4 flex flex-col items-end gap-2 text-xs">
              <div className="flex justify-between w-64">
                <span className="text-text-custom/50 font-semibold">Subtotal:</span>
                <span className="font-bold text-text-custom">${subtotal.toFixed(2)}</span>
              </div>
              <div className="flex justify-between w-64">
                <span className="text-text-custom/50 font-semibold">Sales Tax (8%):</span>
                <span className="font-bold text-text-custom">${tax.toFixed(2)}</span>
              </div>
              <div className="flex justify-between w-64">
                <span className="text-text-custom/50 font-semibold">Shipping Cost:</span>
                <span className="font-bold text-text-custom">${shippingCost.toFixed(2)}</span>
              </div>
              <div className="flex justify-between w-64 border-t border-border-custom/80 pt-2 text-sm">
                <span className="text-text-custom font-extrabold">Final Total:</span>
          ...
          <Card title="Customer Information">
            <div className="space-y-4 mt-2 text-xs">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-text-custom text-sm">{order.customerName}</p>
                  <p className="text-text-custom/50 font-medium">{order.email}</p>
                </div>
              </div>
              ...
              <div className="border-t border-border-custom/50 pt-4 space-y-2.5">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-bold text-text-custom">Shipping Address</p>
                    <p className="text-text-custom/75 mt-0.5">4582 Oakwood Avenue, Suite 100, Los Angeles, CA 90004</p>
                ...
                <div className="flex items-start gap-2 border-t border-border-custom/50 pt-3">
                  ...
                  <div>
                    <p className="font-bold text-text-custom">Settlement Details</p>
                    <p className="text-text-custom/75 mt-0.5">Payment Method: Visa Ending *2834</p>
                    <p className="text-3xs text-text-custom/40 font-bold uppercase tracking-wider mt-1">
                      Payment Status: <span className="font-bold text-text-custom uppercase">{order.paymentStatus}</span>
                    </p>
                  </div>
                </div>
              </div>
      ...
      >
        ...
        <div className="space-y-8 font-sans p-2" id="printable-invoice">
          <div className="flex justify-between items-start">
            <div>
              <span className="text-lg font-bold text-primary tracking-wider">LUUNA LUXURY</span>
              <p className="text-3xs text-text-custom/50">842 Sunset Blvd, Los Angeles, CA 90028</p>
            </div>
            <div className="text-right">
              <span className="text-sm font-extrabold text-text-custom">INVOICE</span>
              <p className="text-3xs font-mono text-text-custom/50">#INV-{order.id}</p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-4 border-t border-border-custom pt-4 text-xs">
            <div>
              <p className="font-bold text-text-custom/50 uppercase tracking-wider text-3xs">Billed To</p>
              <p className="font-bold text-text-custom mt-1">{order.customerName}</p>
              <p className="text-text-custom/75 mt-0.5">{order.email}</p>
              <p className="text-text-custom/75">4582 Oakwood Avenue, Suite 100, Los Angeles, CA 90004</p>
            </div>
            <div className="text-right">
              <p className="font-bold text-text-custom/50 uppercase tracking-wider text-3xs">Invoice Details</p>
              <p className="text-text-custom/75 mt-1" suppressHydrationWarning>Date: {new Date(order.date).toLocaleDateString()}</p>
              <p className="text-text-custom/75">Payment Method: Visa Card (*2834)</p>
            </div>
          </div>
            ...
            <tbody className="divide-y divide-border-custom/50 font-medium">
              {orderItems.map(item => (
                <tr key={item.id}>
                  ...
                  <td className="px-4 py-2.5 text-right font-bold">${item.total.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      >
        <form className="space-y-4">
          <Input
            ...
            type="number"
            step="0.01"
            {...register('refundAmount', { valueAsNumber: true, required: 'Refund amount is required' })}
          ...
          <Select
            ...
            {...register('reason')}
            options={[
              { value: 'Customer Return', label: 'Customer Return / Return Request' },
              { value: 'Out of Stock', label: 'Inventory Out of Stock / Unfulfilled' },
              { value: 'Damaged Item', label: 'Damaged Goods / Quality issues' },
              { value: 'Incorrect Item Sent', label: 'Fulfillment Error / Swap' }
            ]}
          />
        </form>
      </Modal>
