...
import { Breadcrumb, Card, StatsCard, Tabs } from "@/components/admin";
import { useAppSelector } from "@/redux/hooks";

export default function AnalyticsPage() {
  const { products, orders, customers } = useAppSelector(
    (state) => state.admin,
  );
  ...
  const monthlyStats = {
    thisMonthSales: 45290.0,
    lastMonthSales: 39820.0,
    salesChange: 13.73,
    thisMonthOrders: 620,
    lastMonthOrders: 580,
    ordersChange: 6.89,
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-2">
          ...
          <div className="p-4 rounded-xl bg-bg-secondary/50 border border-border-custom flex justify-between items-center text-xs">
            <div>
              ...
              <h3 className="text-lg font-bold text-text-custom mt-1">
                ${monthlyStats.thisMonthSales.toLocaleString()}
              </h3>
              <p className="text-3xs text-text-custom/40 mt-1 font-semibold">
                Prior Month: ${monthlyStats.lastMonthSales.toLocaleString()}
              </p>
            </div>
          ...
          <div className="p-4 rounded-xl bg-bg-secondary/50 border border-border-custom flex justify-between items-center text-xs">
            <div>
              ...
              <h3 className="text-lg font-bold text-text-custom mt-1">
                {monthlyStats.thisMonthOrders} orders
              </h3>
              <p className="text-3xs text-text-custom/40 mt-1 font-semibold">
                Prior Month: {monthlyStats.lastMonthOrders}
              </p>
            </div>
            <div className="text-right">
              <span className="inline-flex items-center gap-0.5 text-emerald-600 font-bold bg-emerald-50 px-2 py-1 rounded text-2xs border border-emerald-100">
          ...
          <div className="p-4 rounded-xl bg-bg-secondary/50 border border-border-custom flex justify-between items-center text-xs">
            <div>
              ...
              <h3 className="text-lg font-bold text-text-custom mt-1">
                {monthlyStats.thisMonthCustomers} signups
              </h3>
              <p className="text-3xs text-text-custom/40 mt-1 font-semibold">
                Prior Month: {monthlyStats.lastMonthCustomers}
              </p>
            </div>
            <div className="text-right">
              <span className="inline-flex items-center gap-0.5 text-emerald-600 font-bold bg-emerald-50 px-2 py-1 rounded text-2xs border border-emerald-100">
          ...
          <div className="h-64 flex flex-col justify-end mt-4">
            <div className="flex-1 w-full relative">
              ...
              >
                ...
                <path
                  d="M 0 160 Q 150 80, 300 100 T 600 30 L 600 180 L 0 180 Z"
                  fill="url(#revGrad)"
                />
                <path
                  d="M 0 160 Q 150 80, 300 100 T 600 30"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="3"
                />
        ...
        <Card title="Active Session acquisitions">
          <div className="h-48 flex flex-col justify-end mt-4">
            <div className="flex-1 w-full relative">
              ...
              >
                <path
                  d="M 0 120 C 100 90, 200 110, 300 50 C 350 20, 400 10, 400 10"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="3"
                />
        ...
        <Card title="Best Selling Products" className="lg:col-span-2">
          <div className="space-y-4 mt-2">
            {bestSellers.map((item, idx) => (
              <div
                key={item.sku}
                className="flex items-center justify-between p-3 rounded-xl hover:bg-bg-secondary/50 border border-border-custom/50 text-xs"
              >
                <div className="flex items-center gap-3">
                  ...
                  <img
                    src={item.image}
                    alt={item.name}
                    className="w-10 h-10 object-cover rounded-lg border border-border-custom"
                  />
                  <div>
                    <span className="font-bold text-text-custom block">
                      {item.name}
                    </span>
                    <span className="text-3xs font-mono text-text-custom/50">
                      SKU: {item.sku}
                    </span>
                  </div>
                </div>
                <div className="text-right">
                  <span className="font-extrabold text-text-custom block">
