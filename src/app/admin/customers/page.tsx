...
import { Edit, Trash, Eye, ShieldAlert, UserCheck, Download, Plus } from 'lucide-react';
...
export default function CustomersPage() {
  ...
  const handleToggleDisableStatus = (cust: Customer) => {
    alert(`Status updated: Customer "${cust.name}" is now ${cust.status === 'active' ? 'suspended' : 'activated'}.`);
  };
  ...
  const handleExport = () => {
    alert('Exporting customer registry as CSV document. The download will start shortly.');
  };
  ...
  const columns: Column<Customer>[] = [
      ...
      render: (_, cust) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-3xs text-text-custom/50 font-medium">{cust.email}</p>
          </div>
        </div>
      )
    },
    {
      key: 'phone',
      label: 'Phone number',
      render: (val) => <span className="text-text-custom/80">{val || '—'}</span>
    },
    {
      key: 'status',
      ...
      render: (_, cust) => (
        <ActionMenu
          items={[
            ...
            { 
              label: cust.status === 'active' ? 'Disable Account' : 'Enable Account', 
              icon: cust.status === 'active' ? <ShieldAlert className="w-3.5 h-3.5" /> : <UserCheck className="w-3.5 h-3.5" />, 
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-text-custom mt-1">Customer Accounts</h1>
        </div>
