    ...
    return (
      <div className="w-full space-y-4">
        {Array.from({ length: 5 }).map((_, idx) => (
          <div key={idx} className="flex gap-4 p-4 border border-border-custom bg-white rounded-lg animate-pulse">
      ...
      <table className="w-full text-left border-collapse text-xs text-text-custom">
            ...
            {columns.map((col) => (
              <th 
                ...
                className={`px-6 py-4 select-none ${
                  col.sortable ? 'cursor-pointer hover:bg-border-custom/50 hover:text-text-custom transition-all' : ''
                }`}
        ...
        <tbody className="divide-y divide-border-custom/50 font-medium">
          {data.map((item) => (
            ...
            >
              {columns.map((col) => {
                const cellValue = col.key !== 'actions' ? item[col.key as keyof T] : undefined;
                return (
                  <td key={String(col.key)} className="px-6 py-4 text-xs align-middle">
